...
@section('main_content')
    <div class="container py-3">
        ...
        <div class="d-flex gap-4">
            <button class="btn btn-danger" onclick="showModal({{$tvShow['id']}})">Elimina</button>
            <a href="{{route('tv_series.edit', $movie['id'])}}" class="btn btn-secondary">Modifica</a>
        </div>
        <div class="position-absolute w-100 hv-100 bg-danger" id="modal{{$tvShow['id']}}" style="display: none; top:0; bottom:0; right:0; left:0; z-index:10;">
            <div class="w-50 m-auto p-3 bg-light rounded mt-5">
                <h3>Sei sicuro di voler eliminare "{{$tvShow['title']}}" dal database?</h3>
                <h4>Non potrai recuperarlo in alcun modo!</h4>
                <div class="d-flex gap-4 align-items-center">
                    <form action="{{ route('tv_series.destroy', $tvShow['id']) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <input type="submit" value="Sì, elimina" class="btn btn-danger mt-3">
                    </form>
                    <button class="btn btn-success my-3" onclick="showModal({{$tvShow['id']}})">No, annulla</button>
                </div>
            </div>
        </div>
    </div>
